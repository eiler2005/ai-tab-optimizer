...
import {
  getLocalSettingsMirror,
  getSettings,
  saveLocalSettingsMirror,
  saveSettings,
} from './persistence';
import {
  fetchLocalServer,
  fetchLocalServerWithTimeout,
...
async function fetchServerJson<T>(
  ...
  return fetchServerJsonInternal<T>(
    endpoint,
    init,
    serverUrl ?? { getSettings: async () => ({ localServerUrl: (await getSettings()).localServerUrl }) },
  );
}
